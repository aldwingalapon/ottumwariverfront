...
if ( ! class_exists( 'Innomerce_Woo_Module' ) ) {

	/**
	 * Define Innomerce_Woo_Module class
	 */
	class Innomerce_Woo_Module extends Innomerce_Module_Base {
		...
		public function filters() {
			...
			add_filter( 'innomerce-theme/assets-depends/script', array( $this, 'assets_depends_script' ) );
		...
		public function customizer_options( $options ) {
			...
			$new_options = array(
				'woocommerce_accent_color' => array(
					'title'    => esc_html__( 'WooCommerce Accent color', 'innomerce' ),
		...
		public function assets_depends_script( $scripts_depends ) {
			array_push( $scripts_depends, 'innomerce-woo-module-script' );
