...
if ( ! class_exists( 'Innomerce_Module_Base' ) ) {

	/**
	 * Define Innomerce_Module_Base class
	 */
	abstract class Innomerce_Module_Base {
		...
		public function load_child_modules( $childs = array() ) {
			...
			foreach ( $childs as $child => $childs_of_child ) {
				innomerce_theme()->load_module( $child, $childs_of_child );
			}
