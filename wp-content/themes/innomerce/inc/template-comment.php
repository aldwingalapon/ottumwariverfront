 ...
 */
function innomerce_rewrite_comment_item( $_comment, $args, $depth ) {
	global $comment;

	$_comment->innomerce_comment_list_args = $args;
	$comment = $_comment;
 ...
 */
function innomerce_comment_author_avatar( $args = array() ) {
	global $comment;

	if ( ! empty( $comment->innomerce_comment_list_args['avatar_size'] ) ) {
		$size = $comment->innomerce_comment_list_args['avatar_size'];
	}
	 ...
	 * @param array $args   Arguments.
	 */
	return apply_filters( 'innomerce-theme/comments/author-avatar', get_avatar( $comment, $size ), $args );
}
 ...
 */
function innomerce_get_comment_author_link( $args = array() ) {
	/**
	 ...
	 * @param array $args   Arguments.
	 */
	return apply_filters( 'innomerce-theme/comments/author-link', sprintf( '<b class="fn">%s</b>', get_comment_author_link() ), $args );
}
 ...
 */
function innomerce_get_comment_date( $args = array() ) {
	$format = get_option( 'date_format' ) . ' ' . esc_attr_x( '\a\t', 'time prefix in comment date', 'innomerce' ) . ' ' . get_option( 'time_format' );
	 ...
	 * @param array  $args   Arguments.
	 */
	return apply_filters( 'innomerce-theme/comments/date', sprintf( 
 ...
 */
function innomerce_get_comment_text( $args = array() ) {
	 ...
	 * @param array  $args         Arguments.
	 */
	return apply_filters( 'innomerce-theme/comments/text', $comment_text, $args );
}
 ...
 */
function innomerce_get_comment_reply_link( $args = array() ) {
	global $comment_depth;
	 ...
	 * @param array  $args  Arguments.
	 */
	return apply_filters( 'innomerce-theme/comments/reply_link', $reply, $args );
}
 ...
 */
function innomerce_get_comment_link_edit( $args = array() ) {
	global $comment;

	$text = esc_html__( 'Edit', 'innomerce' );
	 ...
	 * @param array  $args       Arguments.
	 */
	return apply_filters( 'innomerce-theme/comments/link-edit', $link, $comment->comment_ID, $args );
}
