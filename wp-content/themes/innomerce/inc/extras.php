 ...
 * Sidebar position
 */
add_filter( 'theme_mod_sidebar_position', 'innomerce_set_post_meta_value' );

/**
 * Container type
 */
add_filter( 'theme_mod_container_type', 'innomerce_set_post_meta_value' );

/**
 * Header layout type
 */
add_filter( 'theme_mod_header_layout_type', 'innomerce_set_header_layout_value' );
 ...
 * @return string
 */
function innomerce_set_post_meta_value( $value ) {
	$queried_obj = innomerce_get_queried_obj();

	if ( ! $queried_obj ) {
		return $value;
	}

	$meta_key   = 'innomerce_' . str_replace( 'theme_mod_', '', current_filter() );
 ...
 * @return string
 */
function innomerce_set_header_layout_value( $value ) {

	if ( wp_is_mobile() ) {
		return 'mobile';
	}

	return innomerce_set_post_meta_value( $value );
}
 ...
 */
function innomerce_get_queried_obj() {
	$queried_obj = apply_filters( 'innomerce-theme/posts/queried_object_id', false );

	if ( ! $queried_obj && ! innomerce_maybe_need_rewrite_mod() ) {
		return false;
	}
 ...
 * @return boolean
 */
function innomerce_maybe_need_rewrite_mod() {
 ...
 * @return string
 */
function innomerce_render_macros( $string ) {

	$macros = apply_filters( 'innomerce-theme/data_macros', array(
		'/%%year%%/' => date( 'Y' ),
 ...
 * @return string
 */
function innomerce_render_icons( $content ) {
	$icons     = innomerce_get_render_icons_set();
	$icons_set = implode( '|', array_keys( $icons ) );

	$regex = '/icon:(' . $icons_set . ')?:?([a-zA-Z0-9-_]+)/';

	return preg_replace_callback( $regex, 'innomerce_render_icons_callback', $content );
}
 ...
 * @return string
 */
function innomerce_render_icons_callback( $matches ) {
 ...
 * @return array
 */
function innomerce_get_render_icons_set() {
	return apply_filters( 'innomerce-theme/icons/icons-set', array(
 ...
 * @return string
 */
function innomerce_render_theme_url( $url ) {
 ...
 * @return string
 */
function innomerce_justify_thumbnail_size( $mask = 0, $thumbnail_size = 'post-thumbnail', $justify_size='innomerce-thumb-justify', $justify_size_1 = 'innomerce-thumb-justify', $justify_size_2 = 'innomerce-thumb-justify-2') {
	$mask_list = array(
 ...
 * @return string
 */
function innomerce_get_post_template_part_slug() {
	return apply_filters( 'innomerce-theme/posts/template-part-slug', 'template-parts/content' );
}
 ...
 * @return string
 */
function innomerce_get_post_style() {
	return apply_filters( 'innomerce-theme/posts/post-style', false );
}
 ...
 * @return array
 */
function innomerce_kses_post_allowed_html( $additional_allowed_html = array() ) {
