 ...
 * @return array
 */
function innomerce_related_posts() {

	if ( ! is_singular( 'post' ) ) {
		return;
	}

	$visible = innomerce_theme()->customizer->get_value( 'related_posts_visible' );
	...
	$post_terms  = array();
	$post_number = innomerce_theme()->customizer->get_value( 'related_posts_count' );
	...
	foreach ( $settings as $setting_key => $setting_value ) {
		$settings[ $setting_key ] = innomerce_theme()->customizer->get_value( $setting_value );
	}
	...
	if ( $holder_view_dir ) {
		...
		foreach ( $posts as $post ) {
			...
			$image = ( $settings['image_visible'] ) ? innomerce_post_thumbnail( 'innomerce-thumb-s', array( 'echo' => false ) ) : '';
			...
			$author = ( $settings['author_visible'] ) ? innomerce_posted_by( array( 'echo' => false ) ) : '';

			$date = ( $settings['date_visible'] ) ? innomerce_posted_on( array( 'echo' => false ) ) : '';
